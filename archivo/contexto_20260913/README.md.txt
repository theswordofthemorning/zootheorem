...
scripts). The [current research plan](PLAN_INVESTIGACION.md) separates
these results from the open density argument and records its next steps.

## Contents

| path | what it is |
|---|---|
| `papers/01-merging-classes.md` | The sequences a(n), b(n): merging classes of consecutive integers under the shortcut Collatz map. Definition, the doubling theorem, the density lower bound 0.435658 for balanced merging, the mirror theorem (3x−1 gives the identical sequence), and the verification provenance of every term. |
| `papers/02-generalized-maps.md` | The same count for (5x+1)/2 and (7x+1)/2 — finite exact data — plus two theorems: the criticality E[m] = (1+q)/4 (q = 3 uniquely has mean shared-step multiplier one) and the obstruction theorem (if an odd prime divides r but not q, consecutive pairs never merge; verified as a falsifiable prediction on three maps). |
| `papers/03-uniform-fractional-moment-bound.md` | A self-contained lemma: uniform-in-time fractional moment bounds when E[multiplier] = 1, with explicit hypotheses, with explicit constants, by subadditivity alone. |
| `PLAN_INVESTIGACION.md` | Current target, corrected density interpretation, open steps P1–P3 and publication criteria. |
...
python verify/verify_strata.py              # e8, e10, e12: all n = 1..20
python verify/verify_consistency.py         # stored lists, b-files and constants
...
A398792 through A398795 are approved and published. The three chain strata were
re-proposed on 2026-09-13 and were last observed as `proposed`: A399819 (`e8`), A399820
(`e10`) and A399821 (`e12`), whose submitted text is kept verbatim in
`drafts/A399819.txt`, `drafts/A399820.txt` and `drafts/A399821.txt`.
See [the revision note](drafts/REVISION_20260913.md) for corrected offsets,
verification ranges and a further clarification prepared for A398792. The
